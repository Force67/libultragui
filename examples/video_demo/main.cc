...
static void draw_cube(ugui::RHI* rhi, ugui::RHITextureHandle tex,
                       float cx, float cy, float size, float time) {
    float yaw = time * 0.7f;
    float pitch = time * 0.4f;
    float fov = 3.0f;

    struct Proj { float x, y, z; };
    Proj proj[8];
    for (int i = 0; i < 8; i++) {
    ...
    for (int fi = 0; fi < 6; fi++) {
        ...
        ugui::u32 col = (0xFFu << 24) | shade | (ugui::u32(shade) << 8) | (ugui::u32(shade) << 16);
...
int main(int argc, char* argv[]) {
    ...
    if (font == ugui::kInvalidFont) { ui.Shutdown(); return 1; }
    ...
    std::printf("Video: %ux%u, %.1f fps, %.1fs duration\n",
                ...
                video->frame_rate(), video->duration());
#else
    ...
    while (ui.Running()) {
        ...
        if (sp2 && !sp) { if (video->IsPlaying()) video->Pause(); else video->Play(); }
        if (rk2 && !rk) { video->Stop(); video->Play(); }
        if (lk2 && !lk) { video->set_loop(!video->IsLooping()); }
        if (lft2 && !lft) { video->Seek(video->position() - 2.0); }
        if (rgt2 && !rgt) { video->Seek(video->position() + 2.0); }
        sp = sp2; rk = rk2; lk = lk2; lft = lft2; rgt = rgt2;
        ...
        if (scene == 0) {
            ...
            if (td) {
                ...
                char buf[64];
                std::snprintf(buf, sizeof(buf), "%d:%02d / %d:%02d%s%s",
