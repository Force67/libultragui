...
static void DrawCube(ugui::RHI* rhi, float time, float screen_cx, float screen_cy) {
    float scale = 120.0f;
    float focal = 600.0f;
    ...
    for (int i = 0; i < 8; ++i) {
        Vec3 v{kCubeVerts[i].x * scale, kCubeVerts[i].y * scale, kCubeVerts[i].z * scale};
        v = RotateX(v, rot_x);
        v = RotateY(v, rot_y);
        xformed[i] = v;
    ...
    for (int f = 0; f < 6; ++f) {
        ...
        draw_order[f] = {f, (xformed[face.idx[0]].z + xformed[face.idx[1]].z +
                             xformed[face.idx[2]].z + xformed[face.idx[3]].z) * 0.25f};
    }
    std::sort(draw_order, draw_order + 6,
    ...
    for (auto& edge : kCubeEdges) {
        ...
        ugui::u32 base = vert_count;
        auto& v0 = all_verts[vert_count++]; v0 = {};
        v0.pos[0] = x0 + nx; v0.pos[1] = y0 + ny; v0.color = edge_color; v0.color2 = edge_color;
        auto& v1 = all_verts[vert_count++]; v1 = {};
        v1.pos[0] = x1 + nx; v1.pos[1] = y1 + ny; v1.color = edge_color; v1.color2 = edge_color;
        auto& v2 = all_verts[vert_count++]; v2 = {};
        v2.pos[0] = x1 - nx; v2.pos[1] = y1 - ny; v2.color = edge_color; v2.color2 = edge_color;
        auto& v3 = all_verts[vert_count++]; v3 = {};
        v3.pos[0] = x0 - nx; v3.pos[1] = y0 - ny; v3.color = edge_color; v3.color2 = edge_color;
...
static void DrawStars(ugui::RHI* rhi, float w, float h, float time) {
    ...
    for (int i = 0; i < kStarCount; ++i) {
        ...
        int vi = i * 4;
        verts[vi] = {}; verts[vi].pos[0] = fx;        verts[vi].pos[1] = fy;
        verts[vi].color = col; verts[vi].color2 = col;
        verts[vi+1] = {}; verts[vi+1].pos[0] = fx+size; verts[vi+1].pos[1] = fy;
        verts[vi+1].color = col; verts[vi+1].color2 = col;
        verts[vi+2] = {}; verts[vi+2].pos[0] = fx+size; verts[vi+2].pos[1] = fy+size;
        verts[vi+2].color = col; verts[vi+2].color2 = col;
        verts[vi+3] = {}; verts[vi+3].pos[0] = fx;      verts[vi+3].pos[1] = fy+size;
        verts[vi+3].color = col; verts[vi+3].color2 = col;
...
int main(int argc, char* argv[]) {
    ...
    if (!font_path) {
        std::fprintf(stderr, "No font found. Set ULTRAGUI_FONT or pass a TTF path.\n");
        return 1;
    }
    std::printf("Font: %s\n", font_path);
    ...
    std::printf("Game overlay demo running. The 3D cube is the 'game'; HUD is libultragui.\n");
