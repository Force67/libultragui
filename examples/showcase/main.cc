...
static const char* find_font() {
  ...
  if (env_font && fs::exists(env_font)) return env_font;
...
static void nerv_setup(ugui::UIContext& ui) {
  ...
  ui.SetOnPaint([](ugui::Renderer2D& r, ugui::RHI* rhi) {
    ...
    r.DrawShadow({gx, gy, gw, gh}, ugui::Color::FromHex(0xff6600, 0.08f), 50.0f,
                 0.0f, {0, 0}, ugui::Vertex2D::PackRadii(12, 12, 12, 12));
...
static void load_scene(ugui::UIContext& ui, int idx) {
  ...
  if (nerv_target != ugui::kInvalidTexture) nerv_teardown(ui);
  ...
  if (idx == NERV_SCENE) nerv_setup(ui);
...
int main(int argc, char* argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    ...
    if (!is_scene) font_path = argv[i];
  }

  if (!font_path) font_path = find_font();
  if (!font_path) {
    std::fprintf(
        stderr,
        "No font found. Set ULTRAGUI_FONT or pass a TTF path.\n"
        "  Usage: %s [dashboard|neon|glass|terminal|rpg|nerv|cosmos|meridian] "
        "[font.ttf]\n",
        argv[0]);
    return 1;
  }
  ...
  std::printf(
      "Press 1-8 to switch scenes: dashboard, neon, glass, terminal, rpg, "
      "nerv, cosmos, meridian\n");
  ...
  while (ui.Running()) {
    ...
    if (current_scene == NERV_SCENE && nerv_target != ugui::kInvalidTexture) {
      ui.QueueOffscreen(nerv_target, ui.root(), ugui::Color::FromHex(0x0a0a18));
    }
