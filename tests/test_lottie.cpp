...
TEST(load_from_data_no_rhi) {
    ...
    // should work for parsing but texture will be INVALID_TEXTURE
...
TEST(default_state) {
    ...
    ASSERT(anim.texture() == ugui::INVALID_TEXTURE);
    ASSERT(anim.width() == 0);
...
TEST(load_from_data) {
    ...
    ugui::LottieAnimation anim;
    bool ok = anim.load_data(nullptr, LOTTIE_JSON, "test", 64, 64);
    ASSERT(ok);
    ASSERT(anim.is_loaded());
    ASSERT(anim.texture() == ugui::INVALID_TEXTURE); // no RHI -> no texture
...
TEST(playback_state_machine) {
    ugui::LottieAnimation anim;
    anim.load_data(nullptr, LOTTIE_JSON, "test2", 32, 32);
    ASSERT(!anim.is_playing());
...
TEST(speed_and_loop) {
    ugui::LottieAnimation anim;
    anim.load_data(nullptr, LOTTIE_JSON, "test3", 32, 32);
...
TEST(seek_progress) {
    ugui::LottieAnimation anim;
    anim.load_data(nullptr, LOTTIE_JSON, "test4", 32, 32);
...
TEST(update_advances_time) {
    ugui::LottieAnimation anim;
    anim.load_data(nullptr, LOTTIE_JSON, "test5", 32, 32);
...
TEST(loop_wraps_around) {
    ugui::LottieAnimation anim;
    anim.load_data(nullptr, LOTTIE_JSON, "test6", 32, 32);
...
TEST(no_loop_stops_at_end) {
    ugui::LottieAnimation anim;
    anim.load_data(nullptr, LOTTIE_JSON, "test7", 32, 32);

    anim.set_loop(false);
    anim.play();
    anim.update(3.0); // past the end
    ASSERT(!anim.is_playing()); // should auto-stop
    ASSERT(anim.progress() > 0.95f);
}
