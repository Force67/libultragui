...
static void get_pixel(const ugui::SvgImage& img, ugui::u32 x, ugui::u32 y,
                      ugui::u8& r, ugui::u8& g, ugui::u8& b, ugui::u8& a) {
...
TEST(parse_simple_rect) {
  ...
  ugui::SvgImage img;
  bool ok = ugui::LoadSvgMemory(svg, std::strlen(svg), img);
  ASSERT(ok);
...
TEST(viewbox_scaling) {
  ...
  ugui::SvgImage img;
  bool ok = ugui::LoadSvgMemory(svg, std::strlen(svg), img, 100, 100);
  ASSERT(ok);
  ASSERT(img.width == 100);
  ASSERT(img.height == 100);
  // Rect at 2,2 size 6x6 in viewBox 0-10 -> should be at 20,20 size 60x60 in
  // 100x100
  ASSERT(has_content(img, 50, 50));
  ASSERT(!has_content(img, 5, 5));
}
...
TEST(target_size_override) {
  ...
  ugui::SvgImage img;
  bool ok = ugui::LoadSvgMemory(svg, std::strlen(svg), img, 200, 200);
  ASSERT(ok);
  ASSERT(img.width == 200);
...
TEST(empty_svg_fails) {
  const char* svg = "";
  ugui::SvgImage img;
  bool ok = ugui::LoadSvgMemory(svg, 0, img);
  ASSERT(!ok);
}
...
TEST(complex_icon_svg) {
  ...
  ugui::SvgImage img;
  bool ok = ugui::LoadSvgMemory(svg, std::strlen(svg), img, 64, 64);
  ASSERT(ok);
  ASSERT(img.width == 64);
  ASSERT(img.height == 64);
  // fill=none so center is empty, but stroked circle at r=3 (scaled to ~8px
  // radius) Check the stroke ring: pixel at ~8px from center (32,32)
  bool found = false;
  for (ugui::u32 x = 20; x < 44 && !found; ++x)
    for (ugui::u32 y = 20; y < 44 && !found; ++y)
      if (has_content(img, x, y)) found = true;
  ASSERT(found);
}
