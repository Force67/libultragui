...
bool LoadSvgMemory(const char* data, usize length, SvgImage& out,
  ...
  svg::Document doc;
  if (!svg::ParseSvg(data, length, doc)) return false;

  if (target_width == 0) target_width = static_cast<u32>(doc.width);
  if (target_height == 0) target_height = static_cast<u32>(doc.height);
