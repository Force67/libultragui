...
LottieAnimation::LottieAnimation(LottieAnimation&& other) noexcept
    : impl_(other.impl_) {
  other.impl_ = nullptr;
}
...
void LottieAnimation::Update(f64 dt) {
  if (!impl_ || !impl_->playing || impl_->total_frames == 0) return;
  ...
  if (impl_->looping) {
    // Wrap time
    while (impl_->current_time >= dur) impl_->current_time -= dur;
    while (impl_->current_time < 0) impl_->current_time += dur;
  } else {
...
u32 LottieAnimation::height() const { return impl_ ? impl_->height : 0; }

void LottieAnimation::Play() {
  if (impl_) impl_->playing = true;
}

void LottieAnimation::Pause() {
  if (impl_) impl_->playing = false;
}
...
bool LottieAnimation::IsPlaying() const { return impl_ && impl_->playing; }

bool LottieAnimation::IsLooping() const { return impl_ && impl_->looping; }

f32 LottieAnimation::speed() const { return impl_ ? impl_->speed : 1.0f; }

f64 LottieAnimation::duration() const { return impl_ ? impl_->duration : 0.0; }

f32 LottieAnimation::progress() const {
  if (!impl_ || impl_->duration <= 0) return 0.0f;
