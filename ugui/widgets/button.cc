...
void Button::Measure(f32& out_width, f32& out_height) {
  ...
  if (style_.font_weight != FontWeight::kRegular ||
      style_.font_style != FontStyle::kNormal)
  ...
  auto run =
      te->Shape(resolved, display_label.c_str(),
                static_cast<u32>(display_label.size()), style_.font_size * sc,
...
void Button::OnPaint(Renderer2D& renderer) {
  ...
  if (te && fh != kInvalidFont && !label_.empty()) {
    ...
    if (s.text_shadow_color.a > 0.0f) {
      Vec2 shadow_pos = {x + s.text_shadow_offset.x,
                         y + s.text_shadow_offset.y};
      Color shadow_color =
