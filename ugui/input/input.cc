...
static bool IsInWidgetTree(const Widget* root, const Widget* target) {
  ...
  for (auto* child : root->children()) {
    if (IsInWidgetTree(child, target)) return true;
  }
  return false;
}
...
bool InputRouter::Process(Widget* root) {
  ...
  if (queue.move_count > 0) {
    Vec2 pos = queue.move_events[queue.move_count - 1]
                   .position;  // Use latest position
    mouse_pos_ = pos;
    ...
    if (new_hover != hovered_) {
      if (hovered_) {
        ...
        hovered_->set_widget_state(
            static_cast<WidgetState>(static_cast<u16>(state) &
                                     ~static_cast<u16>(WidgetState::kHovered)));
        if (on_hover_) on_hover_(hovered_, false);
      }
      hovered_ = new_hover;
      if (hovered_) {
        hovered_->set_widget_state(hovered_->widget_state() |
                                   WidgetState::kHovered);
        if (on_hover_) on_hover_(hovered_, true);
...
void InputRouter::set_hover(Widget* widget) {
  ...
  if (hovered_) {
    hovered_->set_widget_state(hovered_->widget_state() |
                               WidgetState::kHovered);
    if (on_hover_) on_hover_(hovered_, true);
    if (platform_) platform_->SetCursor(hovered_->ComputedStyle().cursor);
  } else if (platform_) {
    platform_->SetCursor(Cursor::kAuto);
  }
}
...
void InputRouter::set_focus(Widget* widget) {
  ...
  focused_ = widget;
  if (focused_) {
    focused_->set_widget_state(focused_->widget_state() |
                               WidgetState::kFocused);
  }
}
