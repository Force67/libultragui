...
static svg::Paint make_paint(const String& str, Color animated_color,
                             bool was_animated) {
  svg::Paint p;
  if (was_animated) {
...
static void build_rect_path(svg::Path& path, f32 x, f32 y, f32 w, f32 h,
                            f32 cr) {
  if (cr <= 0) {
    path.MoveTo({x, y});
    path.LineTo({x + w, y});
  ...
  path.CubicTo({x + w, y + h - cr + k}, {x + w - cr + k, y + h},
               {x + w - cr, y + h});
...
// Keyed by pointer to the layer's path_data string (stable for AnimDocument
// lifetime).
...
static svg::Shape build_shape(const AnimLayer& layer,
                              const EvaluatedLayer& eval) {
  svg::Shape shape;
  shape.opacity = eval.opacity;
  shape.fill = make_paint(layer.fill_str, eval.fill_color, eval.fill_animated);
  shape.stroke =
      make_paint(layer.stroke_str, eval.stroke_color, eval.stroke_animated);
  shape.stroke_width = eval.stroke_width;

  switch (layer.shape_type) {
    case AnimShapeType::kRect:
      build_rect_path(shape.path, eval.x, eval.y, eval.w, eval.h,
                      eval.corner_radius);
      break;
    ...
    case AnimShapeType::kPath:
      if (!layer.path_data.empty()) shape.path = get_cached_path(layer);
      break;
  ...
  if (eval.rotation != 0 || eval.scale_x != 1 || eval.scale_y != 1) {
    ...
    f32 pivot_x = (layer.shape_type == AnimShapeType::kRect)
                      ? eval.x + eval.w * 0.5f
                      : eval.cx;
    f32 pivot_y = (layer.shape_type == AnimShapeType::kRect)
                      ? eval.y + eval.h * 0.5f
                      : eval.cy;
...
void render_anim_frame(const AnimDocument& doc, f32 time, u8* pixels, u32 width,
  ...
  render_layers(doc.layers, normalized_t, s_svg_doc,
                svg::Transform::Identity());
