...
static VkSurfaceFormatKHR choose_surface_format(
  ...
  // Prefer sRGB format: the GPU automatically converts linear framebuffer
  // writes to sRGB on output, giving correct gamma-space anti-aliasing and
  // blending.
  for (auto& f : formats) {
...
bool RHI::Impl::Init(const RHIConfig& config) {
  ...
  // Compute DPI scale: ratio of framebuffer pixels to window (screen)
  // coordinates
  {
    int fw, fh, ww, wh;
...
bool RHI::Impl::create_surface() {
  return glfwCreateWindowSurface(instance_, window_, nullptr, &surface_) ==
         VK_SUCCESS;
}
...
bool RHI::Impl::create_swapchain() {
  ...
  u32 fmt_count;
  vkGetPhysicalDeviceSurfaceFormatsKHR(physical_device_, surface_, &fmt_count,
                                       nullptr);
  ...
  for (u32 i = 0; i < image_count; ++i) {
    ...
    if (vkCreateImageView(device_, &vci, nullptr, &swapchain_views_[i]) !=
        VK_SUCCESS)
      return false;
  }
  return true;
...
bool RHI::Impl::create_descriptor_layout() {
  ...
  if (vkCreateDescriptorSetLayout(device_, &ci, nullptr, &desc_set_layout_) !=
      VK_SUCCESS)
    return false;
  ...
  return vkCreateDescriptorPool(device_, &pci, nullptr, &desc_pool_) ==
         VK_SUCCESS;
}
...
bool RHI::Impl::create_pipeline() {
  ...
  if (vkCreatePipelineLayout(device_, &layout_ci, nullptr, &pipeline_layout_) !=
      VK_SUCCESS) {
  ...
  VkResult result = vkCreateGraphicsPipelines(device_, VK_NULL_HANDLE, 1, &pi,
                                              nullptr, &pipeline_);
...
bool RHI::Impl::create_command_resources() {
  for (u32 i = 0; i < MAX_FRAMES; ++i) {
    ...
    if (vkCreateCommandPool(device_, &pci, nullptr, &frames_[i].cmd_pool) !=
        VK_SUCCESS)
      return false;
    ...
    if (vkAllocateCommandBuffers(device_, &ai, &frames_[i].cmd_buffer) !=
        VK_SUCCESS)
      return false;
  }
  return true;
...
bool RHI::Impl::create_sync_objects() {
  ...
  for (u32 i = 0; i < MAX_FRAMES; ++i) {
    if (vkCreateSemaphore(device_, &sci, nullptr,
                          &frames_[i].image_available) != VK_SUCCESS)
      return false;
    if (vkCreateFence(device_, &fci, nullptr, &frames_[i].in_flight) !=
        VK_SUCCESS)
      return false;
  }
  ...
  for (u32 i = 0; i < image_count && i < MAX_SWAPCHAIN_IMAGES; ++i) {
    if (vkCreateSemaphore(device_, &sci, nullptr, &render_finished_[i]) !=
        VK_SUCCESS)
      return false;
  }
...
u32 RHI::Impl::find_memory_type(u32 type_filter,
...
RHITextureHandle RHI::Impl::CreateTexture(u32 width, u32 height,
  ...
  VkMemoryAllocateInfo ai{VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO};
  ai.allocationSize = reqs.size;
  ai.memoryTypeIndex = find_memory_type(reqs.memoryTypeBits,
  ...
  vkCmdCopyBufferToImage(cmd, staging, slot.image,
  ...
  dii.sampler =
      (filter == RHIFilter::kNearest) ? nearest_sampler_ : default_sampler_;
...
bool RHI::Impl::BeginFrame(Color clear_color) {
  ...
  vkCmdPushConstants(f.cmd_buffer, pipeline_layout_, VK_SHADER_STAGE_VERTEX_BIT,
                     0, sizeof(push), push);
...
void RHI::Impl::DrawTriangles(const Vertex2D* vertices, u32 vertex_count,
  ...
  if (tex < MAX_TEXTURES && textures_[tex].in_use) {
    vkCmdBindDescriptorSets(f.cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                            pipeline_layout_, 0, 1, &textures_[tex].descriptor,
                            0, nullptr);
  }

  // Draw
  vkCmdBindVertexBuffers(f.cmd_buffer, 0, 1, &f.vertex_buffer, &vb_byte_offset);
  vkCmdBindIndexBuffer(f.cmd_buffer, f.index_buffer, ib_byte_offset,
...
void RHI::Impl::DrawTextTriangles(const Vertex2D* vertices, u32 vertex_count,
  ...
  vkCmdBindPipeline(f.cmd_buffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                    text_pipeline_);
  ...
  vkCmdBindVertexBuffers(f.cmd_buffer, 0, 1, &f.text_vertex_buffer,
                         &vb_byte_offset);
  vkCmdBindIndexBuffer(f.cmd_buffer, f.text_index_buffer, ib_byte_offset,
                       VK_INDEX_TYPE_UINT32);
  vkCmdDrawIndexed(f.cmd_buffer, index_count, 1, 0, 0, 0);
...
Vec2 RHI::Impl::display_size() const {
  ...
  // Return window/screen coordinates (UI coordinate space), not framebuffer
  // pixels
...
bool RHI::Impl::create_offscreen_render_pass() {
  ...
  return vkCreateRenderPass(device_, &ci, nullptr, &offscreen_render_pass_) ==
         VK_SUCCESS;
}

RHITextureHandle RHI::Impl::CreateRenderTarget(u32 width, u32 height) {
  if (!create_offscreen_render_pass()) return kInvalidTexture;
...
void RHI::Impl::EndOffscreen(RHITextureHandle target) {
  ...
  barrier.dstAccessMask = VK_ACCESS_SHADER_READ_BIT;
  vkCmdPipelineBarrier(f.cmd_buffer,
                       VK_PIPELINE_STAGE_COLOR_ATTACHMENT_OUTPUT_BIT,
                       VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT, 0, 0, nullptr, 0,
                       nullptr, 1, &barrier);
...
bool RHI::Impl::ensure_video_pipeline() {
  if (video_pipeline_ready_) return true;
  ...
  if (vkCreateDescriptorSetLayout(device_, &dsl_ci, nullptr,
                                  &video_desc_set_layout_) != VK_SUCCESS)
    return false;
  ...
  if (vkCreateDescriptorPool(device_, &dpc, nullptr, &video_desc_pool_) !=
      VK_SUCCESS)
    return false;
  ...
  for (u32 i = 0; i < MAX_FRAMES; ++i) {
    ...
    if (vkAllocateDescriptorSets(device_, &dai, &video_desc_sets_[i]) !=
        VK_SUCCESS)
      return false;
  }
  ...
  if (vkCreatePipelineLayout(device_, &plc, nullptr, &video_pipeline_layout_) !=
      VK_SUCCESS)
    return false;
...
void RHI::Impl::Shutdown() {
  ...
  // Textures
  for (u32 i = 0; i < MAX_TEXTURES; ++i) {
    if (textures_[i].in_use) DestroyTexture(i);
  }

  if (default_sampler_) vkDestroySampler(device_, default_sampler_, nullptr);
  if (nearest_sampler_) vkDestroySampler(device_, nearest_sampler_, nullptr);
  ...
  for (u32 i = 0; i < MAX_FRAMES; ++i) {
    ...
    if (f.index_memory) vkFreeMemory(device_, f.index_memory, nullptr);
    ...
    if (f.cmd_pool) vkDestroyCommandPool(device_, f.cmd_pool, nullptr);
  }

  if (video_pipeline_) vkDestroyPipeline(device_, video_pipeline_, nullptr);
  ...
  if (text_pipeline_) vkDestroyPipeline(device_, text_pipeline_, nullptr);
  if (pipeline_layout_)
