...
GL_FUNC(void, glTexImage2D, GLenum, GLint, GLint, GLsizei, GLsizei, GLint,
        GLenum, GLenum, const void*)
GL_FUNC(void, glTexSubImage2D, GLenum, GLint, GLint, GLint, GLsizei, GLsizei,
        GLenum, GLenum, const void*)
...
GL_FUNC(void, glShaderSource, GLuint, GLsizei, const GLchar* const*,
        const GLint*)
...
GL_FUNC(void, glVertexAttribPointer, GLuint, GLint, GLenum, GLboolean, GLsizei,
        const void*)
GL_FUNC(void, glVertexAttribIPointer, GLuint, GLint, GLenum, GLsizei,
        const void*)
...
void RHI::Impl::set_projection(GLuint program, f32 win_w, f32 win_h) {
  ...
  GLint u_translate =
      (program == quad_program_) ? quad_u_translate_ : text_u_translate_;
...
void RHI::Impl::ensure_vertex_capacity(u32 needed) {
  ...
  glBufferData(GL_ARRAY_BUFFER,
               static_cast<GLsizeiptr>(new_cap * sizeof(Vertex2D)), nullptr,
               GL_DYNAMIC_DRAW);
...
void RHI::Impl::ensure_index_capacity(u32 needed) {
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               static_cast<GLsizeiptr>(new_cap * sizeof(u32)), nullptr,
               GL_DYNAMIC_DRAW);
...
bool RHI::Impl::Init(const RHIConfig& config) {
  ...
  glBindBuffer(GL_ARRAY_BUFFER, vertex_buf_);
  glBufferData(GL_ARRAY_BUFFER,
               static_cast<GLsizeiptr>(vertex_capacity_ * sizeof(Vertex2D)),
               nullptr, GL_DYNAMIC_DRAW);

  index_capacity_ = 32768;
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, index_buf_);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               static_cast<GLsizeiptr>(index_capacity_ * sizeof(u32)), nullptr,
               GL_DYNAMIC_DRAW);
  ...
  glBufferData(
      GL_ARRAY_BUFFER,
      static_cast<GLsizeiptr>(text_vertex_capacity_ * sizeof(Vertex2D)),
      nullptr, GL_DYNAMIC_DRAW);

  text_index_capacity_ = 32768;
  glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, text_index_buf_);
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               static_cast<GLsizeiptr>(text_index_capacity_ * sizeof(u32)),
               nullptr, GL_DYNAMIC_DRAW);
...
void RHI::Impl::DrawTriangles(const Vertex2D* vertices, u32 vertex_count,
  ...
  if (vertices == last_quad_verts_ && vertex_count == last_quad_vert_count_) {
    ...
    glBufferSubData(GL_ARRAY_BUFFER, vb_byte_offset,
                    static_cast<GLsizeiptr>(vertex_count * sizeof(Vertex2D)),
                    vertices);
  ...
  // Draw
  glDrawElements(
      GL_TRIANGLES, static_cast<GLsizei>(index_count), GL_UNSIGNED_INT,
...
void RHI::Impl::DrawTextTriangles(const Vertex2D* vertices, u32 vertex_count,
  ...
  if (vertices == last_text_verts_ && vertex_count == last_text_vert_count_) {
    ...
    vb_byte_offset =
        text_vertex_write_pos_ * static_cast<u32>(sizeof(Vertex2D));
...
void RHI::Impl::UpdateTexture(RHITextureHandle handle, const void* pixels) {
  ...
  auto& slot = textures_[handle];
  glBindTexture(GL_TEXTURE_2D, slot.texture);
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, static_cast<GLsizei>(slot.width),
                  static_cast<GLsizei>(slot.height), slot.data_format,
                  slot.data_type, pixels);
}
...
bool RHI::Impl::BeginOffscreen(RHITextureHandle target, Color clear_color) {
  ...
  glBindFramebuffer(GL_FRAMEBUFFER, slot.fbo);
  glViewport(0, 0, static_cast<GLsizei>(slot.width),
             static_cast<GLsizei>(slot.height));
  glScissor(0, 0, static_cast<GLsizei>(slot.width),
            static_cast<GLsizei>(slot.height));
  ...
  glUseProgram(quad_program_);
  glBindVertexArray(vao_);
  set_projection(quad_program_, static_cast<f32>(slot.width),
                 static_cast<f32>(slot.height));
...
bool RHI::Impl::ensure_video_program() {
  if (video_program_) return true;

  video_program_ = compile_program(kVideoVertSrc, kVideoFragSrc);
  if (!video_program_) return false;
...
void RHI::Impl::ConvertVideoFrame(RHITextureHandle target, RHITextureHandle y,
                                  RHITextureHandle cb, RHITextureHandle cr) {
  if (!ensure_video_program()) return;
  if (target >= MAX_TEXTURES || !textures_[target].in_use ||
      !textures_[target].is_render_target)
    return;
