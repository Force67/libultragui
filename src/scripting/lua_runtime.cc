...
struct ScriptRuntime::Impl {
    ...
    static f64 ApplyEasing(f64 t, u8 easing) {
        ...
        case 3: return t < 0.5 ? 2*t*t : 1 - ((-2*t+2)*(-2*t+2))/2; // ease-in-out
...
int ScriptRuntime::Impl::LuaUguiTween(lua_State* L) {
    ...
    if (lua_type(L, 3) == LUA_TSTRING) {
        tw.is_color = true;
        ParseHexColor(lua_tostring(L, 3), tw.from_r, tw.from_g, tw.from_b, tw.from_a);
    ...
    for (size_t i = 0; i < tweens.size(); ) {
        if (tweens[i].widget_name == tw.widget_name && tweens[i].property == tw.property) {
