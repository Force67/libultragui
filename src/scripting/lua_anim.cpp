...
void register_anim_lua(
    ...
    lua.register_function("load_anim", [loader](lua_State* L) -> int {
        ...
        if (anim) lua_pushlightuserdata(L, anim);
        else lua_pushnil(L);
        return 1;
    });

    lua.register_function("anim_play", [](lua_State* L) -> int {
        auto* a = static_cast<VectorAnimation*>(lua_touserdata(L, 1));
        if (a) a->play();
        return 0;
    });

    lua.register_function("anim_pause", [](lua_State* L) -> int {
        auto* a = static_cast<VectorAnimation*>(lua_touserdata(L, 1));
        if (a) a->pause();
        return 0;
    });

    lua.register_function("anim_stop", [](lua_State* L) -> int {
        auto* a = static_cast<VectorAnimation*>(lua_touserdata(L, 1));
        if (a) a->stop();
        return 0;
    });

    lua.register_function("anim_loop", [](lua_State* L) -> int {
        auto* a = static_cast<VectorAnimation*>(lua_touserdata(L, 1));
        if (a) a->set_loop(lua_toboolean(L, 2));
        return 0;
    });

    lua.register_function("anim_speed", [](lua_State* L) -> int {
        auto* a = static_cast<VectorAnimation*>(lua_touserdata(L, 1));
        if (a) a->set_speed(static_cast<f32>(luaL_checknumber(L, 2)));
        return 0;
    });

    lua.register_function("anim_seek", [](lua_State* L) -> int {
        auto* a = static_cast<VectorAnimation*>(lua_touserdata(L, 1));
        if (a) a->seek(static_cast<f32>(luaL_checknumber(L, 2)));
        return 0;
    });

    lua.register_function("anim_playing", [](lua_State* L) -> int {
        auto* a = static_cast<VectorAnimation*>(lua_touserdata(L, 1));
        lua_pushboolean(L, a && a->is_playing());
        return 1;
    });

    lua.register_function("anim_progress", [](lua_State* L) -> int {
        auto* a = static_cast<VectorAnimation*>(lua_touserdata(L, 1));
        lua_pushnumber(L, a ? a->progress() : 0);
        return 1;
    });

    lua.register_function("anim_duration", [](lua_State* L) -> int {
        auto* a = static_cast<VectorAnimation*>(lua_touserdata(L, 1));
        lua_pushnumber(L, a ? a->duration() : 0);
        return 1;
    });

    lua.register_function("anim_attach", [find_widget](lua_State* L) -> int {
        auto* a = static_cast<VectorAnimation*>(lua_touserdata(L, 1));
