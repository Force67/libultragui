...
static void arc_to_cubics(Path& path, Vec2 from, f32 rx, f32 ry, f32 x_rotation, bool large_arc,
    ...
    for (i32 i = 0; i < segments; ++i) {
        ...
        path.CubicTo(to_world(ep1x, ep1y), to_world(ep2x, ep2y), to_world(ep3x, ep3y));
        cur_angle = a1;
    }
}
...
static void parse_gradient_stops(const XmlNode& node, Gradient& grad) {
    for (auto& child : node.children) {
        ...
        if (opacity_str)
            stop.color = stop.color.WithAlpha(static_cast<f32>(std::atof(opacity_str)));
