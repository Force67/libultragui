...
struct XmlParser {
  ...
  String read_quoted() {
    ...
    if (!eof()) ++p;  // consume closing quote
    return result;
  }
...
static Color parse_color(const char* s) {
  ...
  for (auto& nc : named) {
    ...
    if (match && !*a && !*b) return Color::FromHex(nc.hex);
  }
  ...
  if (s[0] == '#') {
    ...
    if (len == 3) {
      return Color::FromRgba8(static_cast<u8>(digits[0] * 17),
                              static_cast<u8>(digits[1] * 17),
                              static_cast<u8>(digits[2] * 17));
    }
    if (len == 4) {
      return Color::FromRgba8(
          static_cast<u8>(digits[0] * 17), static_cast<u8>(digits[1] * 17),
          static_cast<u8>(digits[2] * 17), static_cast<u8>(digits[3] * 17));
    }
    if (len == 6) {
      return Color::FromRgba8(static_cast<u8>(digits[0] * 16 + digits[1]),
  ...
  if (std::strncmp(s, "rgb", 3) == 0) {
    ...
    return Color::FromRgba8(static_cast<u8>(Clamp(vals[0], 0, 255)),
...
static Transform parse_transform(const char* s) {
  if (!s || !*s) return Transform::Identity();
...
static void arc_to_cubics(Path& path, Vec2 from, f32 rx, f32 ry, f32 x_rotation,
  ...
  f32 dtheta = angle_between((x1p - cxp) / rx, (y1p - cyp) / ry,
  ...
  for (i32 i = 0; i < segments; ++i) {
    ...
    path.CubicTo(to_world(ep1x, ep1y), to_world(ep2x, ep2y),
                 to_world(ep3x, ep3y));
    cur_angle = a1;
  }
}
  ...
  while (*p) {
    ...
    switch (cmd | 0x20) {  // to lowercase
      ...
      case 'q': {
        ...
        Vec2 c1 = {cur.x + 2.0f / 3.0f * (x1 - cur.x),
...
static Path rect_to_path(f32 x, f32 y, f32 w, f32 h, f32 rx, f32 ry) {
  ...
  if (rx <= 0 && ry <= 0) {
    ...
    path.CubicTo({x + w, y + h - ry + ky}, {x + w - rx + kx, y + h},
                 {x + w - rx, y + h});
...
static void parse_linear_gradient(const XmlNode& node, Document& doc) {
  Gradient grad;
  grad.type = GradientType::kLinear;
...
static void parse_radial_gradient(const XmlNode& node, Document& doc) {
  Gradient grad;
  grad.type = GradientType::kRadial;
  const char* id = node.attr("id");
  if (!id) return;
...
struct ParseCtx {
  ...
  Paint fill = {Paint::kSolid, Color::Black(), {}};
  Paint stroke = {Paint::kNone, Color::Black(), {}};
...
static void apply_attrs(const XmlNode& node, ParseCtx& ctx) {
  ...
  auto Resolve = [&](const char* attr_name,
                     const char* style_val) -> const char* {
    return style_val ? style_val : node.attr(attr_name);
  };

  const char* fill_str = Resolve("fill", sa.fill);
  if (fill_str) ctx.fill = parse_paint(fill_str);
  ...
  if (xform) ctx.transform = ctx.transform * parse_transform(xform);
}
...
static void parse_element(const XmlNode& node, Document& doc,
  ...
  if (node.tag == "rect") {
    ...
    if (w > 0 && h > 0) add_shape(doc, rect_to_path(x, y, w, h, rx, ry), ctx);
  } else if (node.tag == "circle") {
    f32 cx = node.attr_f("cx"), cy = node.attr_f("cy"), r = node.attr_f("r");
    if (r > 0) add_shape(doc, circle_to_path(cx, cy, r), ctx);
  } else if (node.tag == "ellipse") {
    f32 cx = node.attr_f("cx"), cy = node.attr_f("cy");
    f32 rx = node.attr_f("rx"), ry = node.attr_f("ry");
    if (rx > 0 && ry > 0) add_shape(doc, ellipse_to_path(cx, cy, rx, ry), ctx);
    ...
    for (auto& child : node.children) parse_element(child, doc, ctx);
    return;
...
bool ParseSvg(const char* data, usize length, Document& out) {
  ...
  if (width_str) out.width = parse_coord_value(width_str);
  if (height_str) out.height = parse_coord_value(height_str);

  // Fall back to viewBox dimensions
  if (out.width <= 0 && out.view_w > 0) out.width = out.view_w;
  if (out.height <= 0 && out.view_h > 0) out.height = out.view_h;

  // Default if nothing specified
  if (out.width <= 0) out.width = 300;
  if (out.height <= 0) out.height = 150;

  // Parse all children
  ParseCtx ctx;
  apply_attrs(root, ctx);

  for (auto& child : root.children) parse_element(child, out, ctx);
