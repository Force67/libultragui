...
static Color eval_gradient_at(const Gradient& grad, f32 px, f32 py, f32 bbox_x, f32 bbox_y,
                              f32 bbox_w, f32 bbox_h) {
    f32 t = 0;
    ...
    if (grad.type == GradientType::Linear) {
        ...
        f32 len_sq = dx * dx + dy * dy;
        if (len_sq > 0)
            t = ((px - p1.x) * dx + (py - p1.y) * dy) / len_sq;
    } else {
        // Radial
        f32 gcx, gcy, gr;
        if (grad.user_space) {
...
static Color resolve_paint(const Paint& paint, f32 px, f32 py, f32 alpha,
    ...
    if (paint.type == Paint::Solid)
        return paint.color.with_alpha(paint.color.a * alpha);
    if (paint.type == Paint::GradientRef) {
        auto it = doc.gradients.find(paint.gradient_id);
        if (it == doc.gradients.end())
            return Color::black().with_alpha(alpha);
        Color c = eval_gradient_at(it->second, px, py, bbox.x0, bbox.y0, bbox.width(),
                                   bbox.height());
        return c.with_alpha(c.a * alpha);
    }
    return Color::transparent();
}
...
static inline void composite_pixel(u8* dst, Color src) {
    ...
    dst[0] = static_cast<u8>(clamp((src.r * sa + dr * da * (1.0f - sa)) * inv_out_a, 0.0f, 1.0f) *
                             255.0f +
                             0.5f);
    dst[1] = static_cast<u8>(clamp((src.g * sa + dg * da * (1.0f - sa)) * inv_out_a, 0.0f, 1.0f) *
                             255.0f +
                             0.5f);
    dst[2] = static_cast<u8>(clamp((src.b * sa + db * da * (1.0f - sa)) * inv_out_a, 0.0f, 1.0f) *
                             255.0f +
                             0.5f);
...
static void rasterize_edges(const std::vector<Edge>& edges, FillRule rule, u8* pixels, u32 width,
    ...
    for (u32 py = 0; py < height; ++py) {
        ...
        for (u32 px = 0; px < width; ++px) {
            ...
            if (cov > 0.001f) {
                Color c = resolve_paint(paint, px + 0.5f, py + 0.5f, opacity * cov, doc, bbox);
...
void rasterize(const Document& doc, u8* pixels, u32 width, u32 height) {
    ...
    if (doc.view_w > 0 && doc.view_h > 0) {
        ...
        view_xform = Transform::scale(sx, sy) * Transform::translate(-doc.view_x, -doc.view_y);
    ...
    for (auto& shape : doc.shapes) {
        ...
        if (shape.fill.type != Paint::None) {
            ...
            if (!edges.empty()) {
                rasterize_edges(edges, shape.fill_rule, pixels, width, height, shape.fill,
                                shape.opacity * shape.fill_opacity, doc);
            }
        }

        // --- Stroke ---
        if (shape.stroke.type != Paint::None && shape.stroke_width > 0) {
            points.clear();
            subpath_starts.clear();
            edges.clear();
            ...
            if (!edges.empty()) {
                rasterize_edges(edges, FillRule::NonZero, pixels, width, height, shape.stroke,
                                shape.opacity * shape.stroke_opacity, doc);
            }
        }
    }
}
