...
f32 EvalEasing(const Transition& transition, f32 t) {
  if (transition.easing == EasingType::kSpring) {
    t = t < 0.0f ? 0.0f : (t > 1.0f ? 1.0f : t);
    return eval_spring(transition.spring_stiffness, transition.spring_damping,
                       transition.spring_mass, t);
  }
  return EvalEasing(transition.easing, t, transition.bezier);
}
