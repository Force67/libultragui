...
void Animator::StartTransition(u32 widget_id, const Style& from,
                               const Style& to, const Transition& transition,
                               f64 current_time) {
  // Cancel existing transition for this widget
  for (auto& t : transitions_) {
    if (t.widget_id == widget_id) t.active = false;
  }
...
void Animator::Cancel(u32 widget_id) {
  ...
  for (auto& a : animations_) {
    if (a.widget_id == widget_id) a.active = false;
  }
}
...
bool Animator::Update(f64 current_time, ApplyFn apply, void* user_data,
  ...
  for (auto& t : transitions_) {
    ...
    if (done) {
      ...
      if (on_done) on_done(t.widget_id, user_data);
    } else {
      any_active = true;
    }
  }
...
bool Animator::IsAnimating(u32 widget_id) const {
  for (auto& t : transitions_) {
    if (t.widget_id == widget_id && t.active) return true;
  }
  for (auto& a : animations_) {
    if (a.widget_id == widget_id && a.active) return true;
  }
  return false;
}
...
void Animator::EvaluateScrollAnimations(u32 scroll_widget_id, f32 scroll_y,
  ...
  for (auto& sa : scroll_anims_) {
    if (sa.scroll_widget_id != scroll_widget_id) continue;
