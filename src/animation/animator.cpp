...
Style KeyframeAnimation::evaluate(f64 current_time, bool& done) const {
    ...
    f32 eased = eval_easing(easing, local_t);
...
void Animator::start_transition(u32 widget_id, const Style& from, const Style& to,
...
void Animator::start_animation(const KeyframeAnimation& anim, f64 current_time) {
    auto copy = anim;
...
bool Animator::is_animating(u32 widget_id) const {
    for (auto& t : transitions_) {
