...
struct AudioBridge {
    ...
    void PushSamples(const float* interleaved, u32 frame_count) {
        ...
        if (ma_pcm_rb_acquire_write(&ring_buffer, &frames, &write_buf) != MA_SUCCESS)
            return;
        if (frames > 0) {
    ...
    static void DataCallback(ma_device* dev, void* output, const void*,
        ...
        void* read_buf;
        ma_uint32 frames = frame_count;
        if (ma_pcm_rb_acquire_read(&self->ring_buffer, &frames, &read_buf) == MA_SUCCESS &&
            frames > 0) {
    ...
    static void OnVideoFrame(plm_t*, plm_frame_t* frame, void* user) {
        ...
        if (self->y_tex == kInvalidTexture) {
            self->y_tex = self->rhi->CreateTexture(
                self->plane_w, self->plane_h, RHIFormat::kR8Unorm,
                frame->y.data);
        } else {
        ...
        if (self->cb_tex == kInvalidTexture) {
            self->cb_tex = self->rhi->CreateTexture(
                self->chroma_w, self->chroma_h, RHIFormat::kR8Unorm,
                frame->cb.data);
        } else {
...
bool VideoPlayer::Load(RHI* rhi, const char* path, AudioEngine* audio) {
    ...
    if (has_audio && audio) {
        ...
        if (impl_->audio_bridge->Init(sample_rate)) {
            ...
            std::fprintf(stderr, "ultragui/video: audio bridge init failed (non-fatal)\n");
    ...
    if (frame)
        Impl::OnVideoFrame(plm, frame, impl_);
...
void VideoPlayer::Update(f64 dt) {
    if (!impl_ || !impl_->playing || impl_->finished)
        return;
...
bool VideoPlayer::NeedsConvert() const {
    return impl_ && impl_->needs_convert;
}

void VideoPlayer::ConvertFrame() {
    if (!impl_ || !impl_->needs_convert)
        return;
    if (impl_->rgba_target == kInvalidTexture ||
        impl_->y_tex == kInvalidTexture ||
        impl_->cb_tex == kInvalidTexture ||
        impl_->cr_tex == kInvalidTexture)
        return;
    impl_->rhi->ConvertVideoFrame(impl_->rgba_target,
                                   impl_->y_tex, impl_->cb_tex, impl_->cr_tex);
...
u32 VideoPlayer::width() const {
    return impl_ ? impl_->width : 0;
}

u32 VideoPlayer::height() const {
    return impl_ ? impl_->height : 0;
}

f64 VideoPlayer::frame_rate() const {
...
void VideoPlayer::Stop() {
    ...
    if (frame)
        Impl::OnVideoFrame(impl_->plm, frame, impl_);
    plm_rewind(impl_->plm);
}
...
f32 VideoPlayer::speed() const {
    return impl_ ? impl_->speed : 1.0f;
}

f64 VideoPlayer::duration() const {
...
#if ULTRAGUI_AUDIO
    if (impl_ && impl_->audio_bridge)
        impl_->audio_bridge->volume = vol;
#else
    (void)vol;
#endif
}
...
#if ULTRAGUI_AUDIO
    if (impl_ && impl_->audio_bridge)
        return impl_->audio_bridge->volume;
#endif
    return 1.0f;
}
...
#if ULTRAGUI_AUDIO
    if (impl_ && impl_->audio_bridge)
        return impl_->audio_bridge->muted;
#endif
    return false;
}
