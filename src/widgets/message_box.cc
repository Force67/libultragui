...
#include <ultragui/widgets/message_box.h>
...
void MessageBox::Setup(const char* title, const char* message,
    ...
    auto MakeBtn = [&](const char* label, MessageBoxResult result,
                       bool primary) {
        ...
        btn->set_style(bs);
        Style hover = bs;
        hover.background = primary ? Color::FromHex(0x60a5fa)
                                   : Color::FromRgba8(80, 80, 100, 255);
        btn->AddStateOverride(WidgetState::kHovered, hover,
