...
u32 TextInput::NextPos(u32 pos) const {
    ...
    if ((c & 0xE0) == 0xC0) return std::min(pos + 2, static_cast<u32>(text_.size()));
    if ((c & 0xF0) == 0xE0) return std::min(pos + 3, static_cast<u32>(text_.size()));
...
u32 TextInput::PrevPos(u32 pos) const {
    if (pos == 0)
        return 0;
    --pos;
    while (pos > 0 && (static_cast<u8>(text_[pos]) & 0xC0) == 0x80)
        --pos;
    return pos;
}
...
void TextInput::OnUpdate(f64 dt) {
    ...
    if (HasState(state_, WidgetState::kFocused)) {
        ...
        if (was_visible != cursor_visible_)
            MarkPaintDirty();
    }
}
...
void TextInput::OnPaint(Renderer2D& renderer) {
    ...
    if (focused && sel_start_ != sel_end_ && !show_placeholder) {
        ...
        auto text_run = te->Shape(fh, text_.c_str(), static_cast<u32>(text_.size()), s.font_size,
                                  s.letter_spacing, s.line_height_multiplier);
        u32 lo = std::min(sel_start_, sel_end_);
        u32 hi = std::max(sel_start_, sel_end_);
        ...
        renderer.DrawRect({text_x + sel_x0, text_y, sel_x1 - sel_x0, run.line_height}, sel_color);
    }

    // Draw text
    Color text_color = show_placeholder ? Color{0.5f, 0.5f, 0.5f, 0.5f * alpha}
    ...
    if (focused && cursor_visible_ && !show_placeholder) {
        ...
        renderer.DrawRect({cx, text_y + 1.0f, 1.5f, run.line_height - 2.0f}, cursor_color);
    }
