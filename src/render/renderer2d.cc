...
void Renderer2D::DrawRectGradient(Rect rect, Color start_color, Color end_color,
    ...
    // so the shader's mix() is a no-op and GPU bilinear interpolation produces the gradient)
    u32 base = static_cast<u32>(vertices_.size());
    vertices_.push_back({{rect.x, rect.y}, {0, 0}, colors[0], colors[0],
                         corner_radii, 0.0f, {hw, hh}, 0.0f, 0});
    vertices_.push_back({{rect.x + rect.w, rect.y}, {1, 0}, colors[1], colors[1],
                         corner_radii, 0.0f, {hw, hh}, 0.0f, 0});
    vertices_.push_back({{rect.x + rect.w, rect.y + rect.h}, {1, 1}, colors[2], colors[2],
                         corner_radii, 0.0f, {hw, hh}, 0.0f, 0});
    vertices_.push_back({{rect.x, rect.y + rect.h}, {0, 1}, colors[3], colors[3],
                         corner_radii, 0.0f, {hw, hh}, 0.0f, 0});
...
RHITextureHandle Renderer2D::GetRadialGradientTexture(Color center, Color edge) {
    ...
    for (u32 y = 0; y < kSize; ++y) {
        for (u32 x = 0; x < kSize; ++x) {
            ...
            u32 idx = (y * kSize + x) * 4;
            pixels[idx + 0] = static_cast<u8>(Clamp(center.r + (edge.r - center.r) * t, 0.0f, 1.0f) * 255.0f);
            pixels[idx + 1] = static_cast<u8>(Clamp(center.g + (edge.g - center.g) * t, 0.0f, 1.0f) * 255.0f);
            pixels[idx + 2] = static_cast<u8>(Clamp(center.b + (edge.b - center.b) * t, 0.0f, 1.0f) * 255.0f);
            pixels[idx + 3] = static_cast<u8>(Clamp(center.a + (edge.a - center.a) * t, 0.0f, 1.0f) * 255.0f);
        }
    }
...
void Renderer2D::DrawRadialGradient(Rect rect, Color center_color, Color edge_color,
                                     u32 corner_radii) {
...
static Color SampleGradient(const GradientStop* stops, u32 count, f32 t) {
    ...
    for (u32 i = 0; i < count - 1; ++i) {
        if (t >= stops[i].position && t <= stops[i + 1].position) {
            ...
            return Color{
                stops[i].color.r + (stops[i + 1].color.r - stops[i].color.r) * local_t,
                stops[i].color.g + (stops[i + 1].color.g - stops[i].color.g) * local_t,
                stops[i].color.b + (stops[i + 1].color.b - stops[i].color.b) * local_t,
                stops[i].color.a + (stops[i + 1].color.a - stops[i].color.a) * local_t,
            };
        }
    }
...
RHITextureHandle Renderer2D::GetMultiStopGradientTexture(const GradientStop* stops, u32 count,
    ...
    for (u32 y = 0; y < kSize; ++y) {
        for (u32 x = 0; x < kSize; ++x) {
            ...
            if (type == GradientType::kRadial) {
                ...
                if (max_proj > 0.001f)
                    t = Clamp(((u - 0.5f) * dx + (v - 0.5f) * dy) / (max_proj * 2.0f) + 0.5f,
                              0.0f, 1.0f);
                else
                    t = 0.5f;
            }
...
void Renderer2D::DrawMultiStopGradient(Rect rect, const GradientStop* stops, u32 stop_count,
                                        GradientType type, f32 angle_deg, u32 corner_radii) {
...
void Renderer2D::DrawInsetShadow(Rect rect, Color shadow_color, f32 blur, f32 spread,
                                  Vec2 offset, u32 corner_radii) {
    // Inset shadow: shrink the SDF rect by spread, offset it, and use negative softness
    // to signal the shader to fade from edge inward
    Rect shadow_rect = {rect.x + offset.x + spread, rect.y + offset.y + spread,
                        rect.w - spread * 2.0f, rect.h - spread * 2.0f};
    if (shadow_rect.w <= 0.0f || shadow_rect.h <= 0.0f)
        return;
    u32 packed =
        Vertex2D::PackColor(shadow_color.r, shadow_color.g, shadow_color.b, shadow_color.a);
    ...
    u32 inner_radii = Vertex2D::PackRadii(
        std::max(0.0f, static_cast<f32>(tl) - spread),
    ...
    EmitQuad(shadow_rect, packed, packed, inner_radii, -blur, 0.0f, 0, kInvalidTexture);
}
