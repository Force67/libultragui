...
struct LottieAnimation::Impl {
    ...
    RHI* rhi = nullptr;
    RHITextureHandle texture = INVALID_TEXTURE;
    u32 width = 0;
    u32 height = 0;
    ...
    void render_frame(u32 frame_no) {
        ...
        if (texture == INVALID_TEXTURE) {
            texture = rhi->create_texture(width, height, RHIFormat::RGBA8_UNORM, rgba_buf.data());
        } else {
            rhi->update_texture(texture, rgba_buf.data());
        }
...
bool LottieAnimation::load_data(RHI* rhi, const char* json_data, const char* key, u32 w, u32 h) {
    if (impl_)
        unload();
...
bool LottieAnimation::is_loaded() const {
    return impl_ && impl_->anim;
}

RHITextureHandle LottieAnimation::texture() const {
    return impl_ ? impl_->texture : INVALID_TEXTURE;
}
...
void LottieAnimation::unload() {
    if (!impl_)
        return;
    if (impl_->texture != INVALID_TEXTURE && impl_->rhi) {
        impl_->rhi->destroy_texture(impl_->texture);
    }
    delete impl_;
    impl_ = nullptr;
}
