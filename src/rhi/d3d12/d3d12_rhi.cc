...
static VkPresentModeKHR choose_present_mode(const Vector<VkPresentModeKHR>& modes, bool vsync) {
    if (vsync)
...
struct RHI::Impl {
    ...
    void ensure_buffer_capacity(ID3D12Resource*& buf, void*& mapped, u32& capacity,
                                u32 required, u32 stride);
    void wait_for_gpu();
    void upload_texture_data(ID3D12Resource* dest, u32 width, u32 height, u32 pixel_size,
                             const void* pixels, D3D12_RESOURCE_STATES before_state);
...
D3D12_CPU_DESCRIPTOR_HANDLE RHI::Impl::rtv_cpu_handle(u32 index) {
    D3D12_CPU_DESCRIPTOR_HANDLE handle = ID3D12DescriptorHeap_GetCPUDescriptorHandleForHeapStart(rtv_heap_);
    handle.ptr += static_cast<SIZE_T>(index) * rtv_descriptor_size_;
    return handle;
}

D3D12_CPU_DESCRIPTOR_HANDLE RHI::Impl::srv_cpu_handle(u32 index) {
    D3D12_CPU_DESCRIPTOR_HANDLE handle = ID3D12DescriptorHeap_GetCPUDescriptorHandleForHeapStart(srv_heap_);
    handle.ptr += static_cast<SIZE_T>(index) * srv_descriptor_size_;
    return handle;
}

D3D12_GPU_DESCRIPTOR_HANDLE RHI::Impl::srv_gpu_handle(u32 index) {
    D3D12_GPU_DESCRIPTOR_HANDLE handle = ID3D12DescriptorHeap_GetGPUDescriptorHandleForHeapStart(srv_heap_);
...
ID3D12Resource* RHI::Impl::create_upload_buffer(u32 size, void** mapped) {
    ...
    HRESULT hr = ID3D12Device_CreateCommittedResource(device_,
        &heap_props, D3D12_HEAP_FLAG_NONE, &desc,
        D3D12_RESOURCE_STATE_GENERIC_READ, nullptr,
        &IID_ID3D12Resource, reinterpret_cast<void**>(&buf));
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d12: CreateCommittedResource (upload) failed: 0x%08lx\n", hr);
        return nullptr;
    }
...
void RHI::Impl::ensure_buffer_capacity(ID3D12Resource*& buf, void*& mapped, u32& capacity,
                                        u32 required, u32 stride) {
...
void RHI::Impl::set_projection_constants(f32 width, f32 height) {
    ...
    // D3D12 NDC: Y-up (bottom=-1, top=+1). Negate Y to flip to screen coords (Y-down).
    f32 push[4] = {
        2.0f / width,
        -2.0f / height,
        -1.0f,
...
void RHI::Impl::upload_texture_data(ID3D12Resource* dest, u32 width, u32 height, u32 pixel_size,
                                     const void* pixels, D3D12_RESOURCE_STATES before_state) {
    // Get the required layout for the copy
    D3D12_RESOURCE_DESC tex_desc = ID3D12Resource_GetDesc(dest);
    D3D12_PLACED_SUBRESOURCE_FOOTPRINT footprint = {};
    UINT64 total_bytes = 0;
    ID3D12Device_GetCopyableFootprints(device_, &tex_desc, 0, 1, 0, &footprint, nullptr, nullptr, &total_bytes);

    // Create staging buffer
    void* staging_mapped = nullptr;
    ID3D12Resource* staging = create_upload_buffer(static_cast<u32>(total_bytes), &staging_mapped);
    if (!staging)
        return;
    ...
    for (u32 row = 0; row < height; ++row) {
        std::memcpy(dst + row * dst_row_pitch, src + row * src_row_pitch, src_row_pitch);
    }
    ...
    ID3D12GraphicsCommandList_CopyTextureRegion(upload_cmd_list_, &dst_loc, 0, 0, 0, &src_loc, nullptr);
...
bool RHI::Impl::Init(const RHIConfig& config) {
    ...
    if (!create_vkd3d_instance())
        return false;
    if (!create_vk_surface())
        return false;
    ...
    // Create VkFence for swapchain image acquisition
    {
        ...
        if (vkCreateFence(vk_device_, &fci, nullptr, &vk_acquire_fence_) != VK_SUCCESS) {
...
bool RHI::Impl::create_vkd3d_instance() {
    ...
    if (!glfw_exts) {
        std::fprintf(stderr, "ultragui-d3d12: glfwGetRequiredInstanceExtensions failed\n");
        return false;
    }
    ...
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d12: vkd3d_create_instance failed: 0x%08lx\n", hr);
        return false;
    }

    vk_instance_ = vkd3d_instance_get_vk_instance(vkd3d_inst_);
    if (!vk_instance_) {
        std::fprintf(stderr, "ultragui-d3d12: vkd3d_instance_get_vk_instance returned null\n");
        return false;
    }
...
bool RHI::Impl::pick_physical_device() {
    ...
    for (auto dev : devices) {
        ...
        for (u32 i = 0; i < qcount; ++i) {
            if ((qfamilies[i].queueFlags & VK_QUEUE_GRAPHICS_BIT) && !found_graphics) {
                vk_graphics_family_ = i;
                found_graphics = true;
            }
            VkBool32 present_support = false;
            vkGetPhysicalDeviceSurfaceSupportKHR(dev, i, vk_surface_, &present_support);
...
bool RHI::Impl::create_d3d12_device() {
    ...
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d12: vkd3d_create_device failed: 0x%08lx\n", hr);
        return false;
    }
...
bool RHI::Impl::create_vk_swapchain() {
    ...
    u32 fmt_count;
    vkGetPhysicalDeviceSurfaceFormatsKHR(vk_physical_, vk_surface_, &fmt_count, nullptr);
    Vector<VkSurfaceFormatKHR> formats(fmt_count);
    vkGetPhysicalDeviceSurfaceFormatsKHR(vk_physical_, vk_surface_, &fmt_count, formats.data());

    u32 pm_count;
    vkGetPhysicalDeviceSurfacePresentModesKHR(vk_physical_, vk_surface_, &pm_count, nullptr);
    Vector<VkPresentModeKHR> modes(pm_count);
    vkGetPhysicalDeviceSurfacePresentModesKHR(vk_physical_, vk_surface_, &pm_count, modes.data());
    ...
    if (vkCreateSwapchainKHR(vk_device_, &ci, nullptr, &vk_swapchain_) != VK_SUCCESS) {
    ...
    vkGetSwapchainImagesKHR(vk_device_, vk_swapchain_, &swapchain_count_, vk_swapchain_images_);
    ...
    for (u32 i = 0; i < swapchain_count_; ++i) {
        ...
        img_ci.flags = VKD3D_RESOURCE_INITIAL_STATE_TRANSITION | VKD3D_RESOURCE_PRESENT_STATE_TRANSITION;
        img_ci.present_state = D3D12_RESOURCE_STATE_PRESENT;

        HRESULT hr = vkd3d_create_image_resource(device_, &img_ci, &swapchain_resources_[i]);
        if (FAILED(hr)) {
            std::fprintf(stderr, "ultragui-d3d12: vkd3d_create_image_resource[%u] failed: 0x%08lx\n", i, hr);
            return false;
        }
    }
    ...
    rtv_next_index_ = 0;
    for (u32 i = 0; i < swapchain_count_; ++i) {
        ID3D12Device_CreateRenderTargetView(device_, swapchain_resources_[i], nullptr, rtv_cpu_handle(i));
    }
...
bool RHI::Impl::create_command_resources() {
    ...
    HRESULT hr = ID3D12Device_CreateCommandQueue(device_, &queue_desc, &IID_ID3D12CommandQueue,
    ...
    for (u32 i = 0; i < MAX_FRAMES; ++i) {
        hr = ID3D12Device_CreateCommandAllocator(device_, D3D12_COMMAND_LIST_TYPE_DIRECT,
                                              &IID_ID3D12CommandAllocator,
                                              reinterpret_cast<void**>(&cmd_allocs_[i]));
        if (FAILED(hr)) {
            std::fprintf(stderr, "ultragui-d3d12: CreateCommandAllocator[%u] failed\n", i);
            return false;
        }

        hr = ID3D12Device_CreateCommandList(device_, 0, D3D12_COMMAND_LIST_TYPE_DIRECT, cmd_allocs_[i],
                                         nullptr, &IID_ID3D12GraphicsCommandList,
    ...
    // Create fence
    hr = ID3D12Device_CreateFence(device_, 0, D3D12_FENCE_FLAG_NONE, &IID_ID3D12Fence,
    ...
    hr = ID3D12Device_CreateCommandList(device_, 0, D3D12_COMMAND_LIST_TYPE_DIRECT, upload_cmd_alloc_,
                                     nullptr, &IID_ID3D12GraphicsCommandList,
...
bool RHI::Impl::create_descriptor_heaps() {
    // RTV heap: swapchain images + render target textures
    {
        ...
        HRESULT hr = ID3D12Device_CreateDescriptorHeap(device_, &desc, &IID_ID3D12DescriptorHeap,
                                                     reinterpret_cast<void**>(&rtv_heap_));
        if (FAILED(hr)) {
            std::fprintf(stderr, "ultragui-d3d12: CreateDescriptorHeap (RTV) failed\n");
            return false;
        }
        rtv_descriptor_size_ = ID3D12Device_GetDescriptorHandleIncrementSize(device_, D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
    }
    ...
    // SRV heap (shader-visible) for texture descriptors
    {
        ...
        if (FAILED(hr)) {
            std::fprintf(stderr, "ultragui-d3d12: CreateDescriptorHeap (SRV) failed\n");
            return false;
        }
        srv_descriptor_size_ = ID3D12Device_GetDescriptorHandleIncrementSize(device_, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    }
...
bool RHI::Impl::create_root_signature() {
    ...
    if (FAILED(hr)) {
        if (error_blob) {
            std::fprintf(stderr, "ultragui-d3d12: root signature error: %s\n",
    ...
    hr = ID3D12Device_CreateRootSignature(device_, 0, ID3D10Blob_GetBufferPointer(sig_blob), ID3D10Blob_GetBufferSize(sig_blob),
...
bool RHI::Impl::create_pipelines() {
    ...
    D3D12_INPUT_ELEMENT_DESC input_layout[] = {
        ...
        {"COLOR",        1, DXGI_FORMAT_R32_UINT,         0, 20, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
        {"BLENDINDICES", 0, DXGI_FORMAT_R32_UINT,         0, 24, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
        {"BLENDWEIGHT",  0, DXGI_FORMAT_R32_FLOAT,        0, 28, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
        {"TEXCOORD",     1, DXGI_FORMAT_R32G32_FLOAT,    0, 32, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
        {"BLENDWEIGHT",  1, DXGI_FORMAT_R32_FLOAT,        0, 40, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
        {"COLOR",        2, DXGI_FORMAT_R32_UINT,         0, 44, D3D12_INPUT_CLASSIFICATION_PER_VERTEX_DATA, 0},
    };
    ...
    // --- Quad PSO ---
    {
        ...
        HRESULT hr = ID3D12Device_CreateGraphicsPipelineState(device_,
            &pso_desc, &IID_ID3D12PipelineState,
            reinterpret_cast<void**>(&quad_pso_));
        if (FAILED(hr)) {
            std::fprintf(stderr, "ultragui-d3d12: CreateGraphicsPipelineState (quad) failed: 0x%08lx\n", hr);
            return false;
        }
    }
    ...
    // --- Text PSO ---
    {
        ...
        if (FAILED(hr)) {
            std::fprintf(stderr, "ultragui-d3d12: CreateGraphicsPipelineState (text) failed: 0x%08lx\n", hr);
            return false;
        }
    }
...
bool RHI::Impl::create_upload_buffers() {
    for (u32 i = 0; i < MAX_FRAMES; ++i) {
        ...
        fb.index_buf = create_upload_buffer(fb.index_capacity * sizeof(u32),
                                             &fb.index_mapped);
        if (!fb.index_buf)
            return false;
        ...
        fb.text_vertex_buf = create_upload_buffer(fb.text_vertex_capacity * sizeof(Vertex2D),
                                                   &fb.text_vertex_mapped);
        if (!fb.text_vertex_buf)
            return false;

        // Text index buffer
        fb.text_index_capacity = 32768;
        fb.text_index_buf = create_upload_buffer(fb.text_index_capacity * sizeof(u32),
                                                  &fb.text_index_mapped);
...
bool RHI::Impl::ensure_video_pipeline() {
    ...
    if (FAILED(hr)) {
        if (error_blob) {
            std::fprintf(stderr, "ultragui-d3d12: video root sig error: %s\n",
                         static_cast<const char*>(ID3D10Blob_GetBufferPointer(error_blob)));
            ID3D10Blob_Release(error_blob);
        }
        if (sig_blob)
    ...
    hr = ID3D12Device_CreateGraphicsPipelineState(device_,
        &pso_desc, &IID_ID3D12PipelineState,
        reinterpret_cast<void**>(&video_pso_));
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d12: CreateGraphicsPipelineState (video) failed: 0x%08lx\n", hr);
        return false;
    }
...
bool RHI::Impl::AcquireFrame() {
    ...
    if (ID3D12Fence_GetCompletedValue(fence_) < fence_values_[current_frame_]) {
        while (ID3D12Fence_GetCompletedValue(fence_) < fence_values_[current_frame_]) {
            // spin-wait
        }
    }
    ...
    ID3D12GraphicsCommandList_Reset(cmd_lists_[current_frame_], cmd_allocs_[current_frame_], nullptr);
...
void RHI::Impl::DrawTriangles(const Vertex2D* vertices, u32 vertex_count, const u32* indices,
    ...
    if (vertices == last_quad_verts_ && vertex_count == last_quad_vert_count_) {
        ...
        ensure_buffer_capacity(fb.vertex_buf, fb.vertex_mapped, fb.vertex_capacity,
                               fb.vertex_write_pos + vertex_count, sizeof(Vertex2D));
        vb_byte_offset = fb.vertex_write_pos * sizeof(Vertex2D);
        std::memcpy(static_cast<u8*>(fb.vertex_mapped) + vb_byte_offset,
    ...
    std::memcpy(static_cast<u8*>(fb.index_mapped) + ib_byte_offset,
                indices, index_count * sizeof(u32));
    ...
    if (tex < MAX_TEXTURES && textures_[tex].in_use) {
        ID3D12GraphicsCommandList_SetGraphicsRootDescriptorTable(cmd, 1, srv_gpu_handle(textures_[tex].srv_index));
    }

    // Set vertex buffer view
    D3D12_VERTEX_BUFFER_VIEW vbv = {};
    vbv.BufferLocation = ID3D12Resource_GetGPUVirtualAddress(fb.vertex_buf) + vb_byte_offset;
    ...
    ibv.BufferLocation = ID3D12Resource_GetGPUVirtualAddress(fb.index_buf) + ib_byte_offset;
...
void RHI::Impl::DrawTextTriangles(const Vertex2D* vertices, u32 vertex_count, const u32* indices,
    ...
    if (vertices == last_text_verts_ && vertex_count == last_text_vert_count_) {
        ...
        ensure_buffer_capacity(fb.text_vertex_buf, fb.text_vertex_mapped,
                               fb.text_vertex_capacity,
    ...
    std::memcpy(static_cast<u8*>(fb.text_index_mapped) + ib_byte_offset,
                indices, index_count * sizeof(u32));
    ...
    D3D12_VERTEX_BUFFER_VIEW vbv = {};
    vbv.BufferLocation = ID3D12Resource_GetGPUVirtualAddress(fb.text_vertex_buf) + vb_byte_offset;
    vbv.SizeInBytes = vertex_count * sizeof(Vertex2D);
    vbv.StrideInBytes = sizeof(Vertex2D);
    ID3D12GraphicsCommandList_IASetVertexBuffers(cmd, 0, 1, &vbv);

    D3D12_INDEX_BUFFER_VIEW ibv = {};
    ibv.BufferLocation = ID3D12Resource_GetGPUVirtualAddress(fb.text_index_buf) + ib_byte_offset;
    ibv.SizeInBytes = index_count * sizeof(u32);
    ibv.Format = DXGI_FORMAT_R32_UINT;
    ID3D12GraphicsCommandList_IASetIndexBuffer(cmd, &ibv);
...
RHITextureHandle RHI::Impl::CreateTexture(u32 width, u32 height, RHIFormat format,
    ...
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d12: CreateTexture resource failed: 0x%08lx\n", hr);
        return kInvalidTexture;
    }
    ...
    ID3D12Device_CreateShaderResourceView(device_, slot.resource, &srv_desc, srv_cpu_handle(slot.srv_index));
...
void RHI::Impl::UpdateTexture(RHITextureHandle handle, const void* pixels) {
    ...
    auto& slot = textures_[handle];
    upload_texture_data(slot.resource, slot.width, slot.height, slot.pixel_size, pixels,
...
RHITextureHandle RHI::Impl::CreateRenderTarget(u32 width, u32 height) {
    ...
    HRESULT hr = ID3D12Device_CreateCommittedResource(device_,
        ...
        &IID_ID3D12Resource, reinterpret_cast<void**>(&slot.resource));
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d12: CreateRenderTarget resource failed: 0x%08lx\n", hr);
        return kInvalidTexture;
    }
    ...
    ID3D12Device_CreateRenderTargetView(device_, slot.resource, &rtv_desc, rtv_cpu_handle(slot.rtv_index));
...
bool RHI::Impl::BeginOffscreen(RHITextureHandle target, Color clear_color) {
    ...
    set_projection_constants(static_cast<f32>(slot.width), static_cast<f32>(slot.height));
...
void RHI::Impl::ConvertVideoFrame(RHITextureHandle target,
    ...
    ID3D12GraphicsCommandList_SetGraphicsRootDescriptorTable(cmd, 0, srv_gpu_handle(textures_[y].srv_index));
    ID3D12GraphicsCommandList_SetGraphicsRootDescriptorTable(cmd, 1, srv_gpu_handle(textures_[cb].srv_index));
    ID3D12GraphicsCommandList_SetGraphicsRootDescriptorTable(cmd, 2, srv_gpu_handle(textures_[cr].srv_index));
...
void RHI::Impl::Shutdown() {
    ...
    for (u32 i = 0; i < MAX_FRAMES; ++i) {
        if (cmd_lists_[i])
            ID3D12GraphicsCommandList_Release(cmd_lists_[i]);
        if (cmd_allocs_[i])
            ID3D12CommandAllocator_Release(cmd_allocs_[i]);
    }
    if (upload_cmd_list_)
        ID3D12GraphicsCommandList_Release(upload_cmd_list_);
    if (upload_cmd_alloc_)
        ID3D12CommandAllocator_Release(upload_cmd_alloc_);
    if (fence_)
        ID3D12Fence_Release(fence_);
    if (cmd_queue_)
        ID3D12CommandQueue_Release(cmd_queue_);

    // Pipelines
    if (video_pso_)
        ID3D12PipelineState_Release(video_pso_);
    if (video_root_sig_)
        ID3D12RootSignature_Release(video_root_sig_);
    if (text_pso_)
        ID3D12PipelineState_Release(text_pso_);
    if (quad_pso_)
        ID3D12PipelineState_Release(quad_pso_);
    if (root_sig_)
        ID3D12RootSignature_Release(root_sig_);

    // Descriptor heaps
    if (srv_heap_)
        ID3D12DescriptorHeap_Release(srv_heap_);
    if (rtv_heap_)
        ID3D12DescriptorHeap_Release(rtv_heap_);
    ...
    if (device_)
        ID3D12Device_Release(device_);

    // vkd3d instance (releases VkInstance)
    if (vkd3d_inst_)
        vkd3d_instance_decref(vkd3d_inst_);
}
