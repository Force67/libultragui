...
// Use C-style COM interface (DXVK-native is a C library; C++ vtable ABI can mismatch)
#define NOMINMAX
#define INITGUID
#define CINTERFACE
#define COBJMACROS
...
typedef HRESULT (__attribute__((ms_abi)) *PFN_D3DCompile_vkd3d)(
    const void*, SIZE_T, const char*,
    const void*, void*,   // D3D_SHADER_MACRO*, ID3DInclude*
...
static bool load_d3dcompile() {
    ...
    if (!s_D3DCompile) {
        std::fprintf(stderr, "ultragui-d3d11: D3DCompile not found in libvkd3d-utils\n");
        return false;
    }
    return true;
}
...
struct VkD3DBlobVtbl {
    // IUnknown
    HRESULT (__attribute__((ms_abi)) *QueryInterface)(VkD3DBlob*, const IID&, void**);
...
static bool compile_shader(const char* source, const char* entry, const char* target,
    ...
    HRESULT hr = s_D3DCompile(source, std::strlen(source), "shader", nullptr, nullptr,
                               entry, target, 0, 0,
                               reinterpret_cast<ID3D10Blob**>(&blob),
                               reinterpret_cast<ID3D10Blob**>(&errors));
    if (FAILED(hr)) {
        if (errors) {
            std::fprintf(stderr, "ultragui-d3d11: shader compile error (%s/%s):\n%s\n",
                         entry, target,
...
void RHI::Impl::update_projection(f32 width, f32 height) {
    // D3D11 NDC: Y-up (bottom=-1, top=+1). Negate Y to flip to screen coords (Y-down).
    float push[4] = {
        2.0f / width,
        -2.0f / height,
        -1.0f,
    ...
    HRESULT hr = ID3D11DeviceContext_Map(ctx_, (ID3D11Resource*)projection_cb_,
...
bool RHI::Impl::create_shaders() {
    ...
    HRESULT hr = ID3D11Device_CreateVertexShader(device_, bytecode.data(), bytecode.size(), nullptr, &quad_vs_);
    if (FAILED(hr)) { std::fprintf(stderr, "ultragui-d3d11: CreateVertexShader (quad) failed\n"); return false; }
    ...
    hr = ID3D11Device_CreatePixelShader(device_, bytecode.data(), bytecode.size(), nullptr, &quad_ps_);
    if (FAILED(hr)) { std::fprintf(stderr, "ultragui-d3d11: CreatePixelShader (quad) failed\n"); return false; }

    // Text vertex shader
    if (!compile_shader(text_hlsl, "VSMain", "vs_5_0", bytecode)) return false;
    hr = ID3D11Device_CreateVertexShader(device_, bytecode.data(), bytecode.size(), nullptr, &text_vs_);
    if (FAILED(hr)) { std::fprintf(stderr, "ultragui-d3d11: CreateVertexShader (text) failed\n"); return false; }

    // Text pixel shader
    if (!compile_shader(text_hlsl, "PSMain", "ps_5_0", bytecode)) return false;
    hr = ID3D11Device_CreatePixelShader(device_, bytecode.data(), bytecode.size(), nullptr, &text_ps_);
    if (FAILED(hr)) { std::fprintf(stderr, "ultragui-d3d11: CreatePixelShader (text) failed\n"); return false; }
...
bool RHI::Impl::create_input_layout(const void* vs_bytecode, size_t vs_size) {
    D3D11_INPUT_ELEMENT_DESC layout[] = {
        ...
        {"BLENDWEIGHT",  0, DXGI_FORMAT_R32_FLOAT,       0, 28, D3D11_INPUT_PER_VERTEX_DATA, 0},
...
bool RHI::Impl::create_rasterizer_state() {
    ...
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d11: CreateRasterizerState failed: 0x%08lx\n", hr);
        return false;
    }
    return true;
}
...
bool RHI::Impl::create_samplers() {
    // Linear clamp sampler (slot 0)
    {
        ...
        HRESULT hr = ID3D11Device_CreateSamplerState(device_, &sd, &sampler_linear_);
        if (FAILED(hr)) {
            std::fprintf(stderr, "ultragui-d3d11: CreateSamplerState (linear) failed: 0x%08lx\n", hr);
            return false;
        }
    }
    ...
    // Nearest clamp sampler (slot 1)
    {
        ...
        HRESULT hr = ID3D11Device_CreateSamplerState(device_, &sd, &sampler_nearest_);
        if (FAILED(hr)) {
            std::fprintf(stderr, "ultragui-d3d11: CreateSamplerState (nearest) failed: 0x%08lx\n", hr);
            return false;
        }
    }
...
bool RHI::Impl::create_projection_cb() {
    ...
    HRESULT hr = ID3D11Device_CreateBuffer(device_, &desc, nullptr, &projection_cb_);
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d11: CreateBuffer (projection CB) failed: 0x%08lx\n", hr);
        return false;
    }
    return true;
}
...
bool RHI::Impl::create_default_resources() {
    ...
    if (white_texture_ == kInvalidTexture) {
        std::fprintf(stderr, "ultragui-d3d11: failed to create white fallback texture\n");
        return false;
    }
    return true;
}
...
bool RHI::Impl::ensure_video_shaders() {
    ...
    HRESULT hr = ID3D11Device_CreateVertexShader(device_, bytecode.data(), bytecode.size(), nullptr, &video_vs_);
    if (FAILED(hr)) { std::fprintf(stderr, "ultragui-d3d11: CreateVertexShader (video) failed\n"); return false; }

    if (!compile_shader(video_hlsl, "PSMain", "ps_5_0", bytecode)) return false;
    hr = ID3D11Device_CreatePixelShader(device_, bytecode.data(), bytecode.size(), nullptr, &video_ps_);
    if (FAILED(hr)) { std::fprintf(stderr, "ultragui-d3d11: CreatePixelShader (video) failed\n"); return false; }
...
static bool create_backbuffer_rtv(ID3D11Device* device, IDXGISwapChain* swapchain,
    ...
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d11: CreateRenderTargetView failed: 0x%08lx\n", hr);
        return false;
    }
    return true;
}
...
bool RHI::Impl::Init(const RHIConfig& config) {
    ...
    sc_desc.OutputWindow = (HWND)window_;  // GLFWwindow* cast for DXVK-native GLFW WSI
    ...
    hr = ID3D11Device_QueryInterface(device_, IID_IDXGIDevice, (void**)&dxgi_device);
    if (FAILED(hr)) {
    ...
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d11: GetParent for IDXGIFactory failed: 0x%08lx\n", hr);
        return false;
    }

    std::fprintf(stderr, "ultragui-d3d11: got DXGI factory, creating swapchain\n");

    hr = IDXGIFactory_CreateSwapChain(factory, (IUnknown*)device_, &sc_desc, &swapchain_);
...
bool RHI::Impl::BeginFrame(Color clear_color) {
    ...
    if (framebuffer_resized_) {
        ...
        HRESULT hr = IDXGISwapChain_ResizeBuffers(swapchain_, 0,
                                                    swapchain_width_, swapchain_height_,
    ...
    ID3D11DeviceContext_OMSetBlendState(ctx_, blend_state_, blend_factor, 0xFFFFFFFF);
    ...
    // Set topology
    ID3D11DeviceContext_IASetPrimitiveTopology(ctx_, D3D11_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
...
void RHI::Impl::DrawTriangles(const Vertex2D* vertices, u32 vertex_count, const u32* indices,
    ...
    if (vertices == last_quad_verts_ && vertex_count == last_quad_vert_count_) {
        ...
        D3D11_MAPPED_SUBRESOURCE mapped = {};
        HRESULT hr = ID3D11DeviceContext_Map(ctx_, (ID3D11Resource*)vertex_buf_,
                                              0, map_type, 0, &mapped);
        if (SUCCEEDED(hr)) {
            std::memcpy(static_cast<u8*>(mapped.pData) + vb_byte_offset,
    ...
    ensure_buffer(index_buf_, index_capacity_,
    ...
    u32 ib_byte_offset = index_write_pos_ * sizeof(u32);
    {
        D3D11_MAP map_type = (ib_byte_offset == 0) ? D3D11_MAP_WRITE_DISCARD
                                                    : D3D11_MAP_WRITE_NO_OVERWRITE;
        D3D11_MAPPED_SUBRESOURCE mapped = {};
        HRESULT hr = ID3D11DeviceContext_Map(ctx_, (ID3D11Resource*)index_buf_,
                                              0, map_type, 0, &mapped);
        if (SUCCEEDED(hr)) {
            std::memcpy(static_cast<u8*>(mapped.pData) + ib_byte_offset,
    ...
    ID3D11DeviceContext_IASetVertexBuffers(ctx_, 0, 1, &vertex_buf_, &stride, &offset);

    // Bind index buffer
    ID3D11DeviceContext_IASetIndexBuffer(ctx_, index_buf_, DXGI_FORMAT_R32_UINT, ib_byte_offset);
...
void RHI::Impl::DrawTextTriangles(const Vertex2D* vertices, u32 vertex_count, const u32* indices,
    ...
    if (vertices == last_text_verts_ && vertex_count == last_text_vert_count_) {
        ...
        D3D11_MAP map_type = first_text_draw_ ? D3D11_MAP_WRITE_DISCARD
                                              : D3D11_MAP_WRITE_NO_OVERWRITE;
        D3D11_MAPPED_SUBRESOURCE mapped = {};
        HRESULT hr = ID3D11DeviceContext_Map(ctx_, (ID3D11Resource*)text_vertex_buf_,
                                              0, map_type, 0, &mapped);
        if (SUCCEEDED(hr)) {
            std::memcpy(static_cast<u8*>(mapped.pData) + vb_byte_offset,
                        vertices, vertex_count * sizeof(Vertex2D));
    ...
    ID3D11DeviceContext_IASetVertexBuffers(ctx_, 0, 1, &text_vertex_buf_, &stride, &vb_offset);

    // Bind text index buffer
    ID3D11DeviceContext_IASetIndexBuffer(ctx_, text_index_buf_, DXGI_FORMAT_R32_UINT, ib_byte_offset);

    // Bind atlas texture SRV to PS slot 0
    if (atlas_texture < MAX_TEXTURES && textures_[atlas_texture].in_use) {
        ID3D11DeviceContext_PSSetShaderResources(ctx_, 0, 1, &textures_[atlas_texture].srv);
    }
...
RHITextureHandle RHI::Impl::CreateTexture(u32 width, u32 height, RHIFormat format,
    ...
    HRESULT hr = ID3D11Device_CreateTexture2D(device_, &tex_desc,
                                               pixels ? &init_data : nullptr,
                                               &slot.texture);
    if (FAILED(hr)) {
    ...
    hr = ID3D11Device_CreateShaderResourceView(device_, (ID3D11Resource*)slot.texture,
                                                nullptr, &slot.srv);
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d11: CreateShaderResourceView failed: 0x%08lx\n", hr);
...
void RHI::Impl::UpdateTexture(RHITextureHandle handle, const void* pixels) {
    ...
    auto& slot = textures_[handle];
    ID3D11DeviceContext_UpdateSubresource(ctx_, (ID3D11Resource*)slot.texture,
                                          0, nullptr, pixels,
...
RHITextureHandle RHI::Impl::CreateRenderTarget(u32 width, u32 height) {
    ...
    HRESULT hr = ID3D11Device_CreateTexture2D(device_, &tex_desc, nullptr, &slot.texture);
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d11: CreateTexture2D (RT) failed: 0x%08lx\n", hr);
        return kInvalidTexture;
    }
    ...
    hr = ID3D11Device_CreateShaderResourceView(device_, (ID3D11Resource*)slot.texture,
                                                nullptr, &slot.srv);
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d11: CreateShaderResourceView (RT) failed: 0x%08lx\n", hr);
        ID3D11Texture2D_Release(slot.texture);
        slot.texture = nullptr;
        return kInvalidTexture;
    }
    ...
    // Create RTV
    hr = ID3D11Device_CreateRenderTargetView(device_, (ID3D11Resource*)slot.texture,
                                              nullptr, &slot.rtv);
    if (FAILED(hr)) {
        std::fprintf(stderr, "ultragui-d3d11: CreateRenderTargetView (RT) failed: 0x%08lx\n", hr);
        ID3D11ShaderResourceView_Release(slot.srv);
        ID3D11Texture2D_Release(slot.texture);
        slot.srv = nullptr;
        slot.texture = nullptr;
...
void RHI::Impl::DestroyRenderTarget(RHITextureHandle handle) {
    ...
    auto& slot = textures_[handle];
    if (slot.rtv)
        ID3D11RenderTargetView_Release(slot.rtv);
    if (slot.srv)
        ID3D11ShaderResourceView_Release(slot.srv);
    if (slot.texture)
        ID3D11Texture2D_Release(slot.texture);
...
bool RHI::Impl::BeginOffscreen(RHITextureHandle target, Color clear_color) {
    ...
    // Unbind SRV if this texture is currently bound (D3D11 validation requires it)
    ...
    update_projection(static_cast<f32>(slot.width), static_cast<f32>(slot.height));
...
void RHI::Impl::Shutdown() {
    ...
    if (ctx_)
        ID3D11DeviceContext_Flush(ctx_);
    ...
    for (u32 i = 0; i < MAX_TEXTURES; ++i) {
        ...
        if (textures_[i].is_render_target) {
            if (textures_[i].rtv)
                ID3D11RenderTargetView_Release(textures_[i].rtv);
        }
        if (textures_[i].srv)
            ID3D11ShaderResourceView_Release(textures_[i].srv);
        if (textures_[i].texture)
            ID3D11Texture2D_Release(textures_[i].texture);
        textures_[i] = {};
    }
    ...
    // Dynamic buffers
    if (vertex_buf_)
        ID3D11Buffer_Release(vertex_buf_);
    if (index_buf_)
        ID3D11Buffer_Release(index_buf_);
    if (text_vertex_buf_)
        ID3D11Buffer_Release(text_vertex_buf_);
    if (text_index_buf_)
        ID3D11Buffer_Release(text_index_buf_);

    // Projection CB
    if (projection_cb_)
        ID3D11Buffer_Release(projection_cb_);

    // Samplers
    if (sampler_linear_)
        ID3D11SamplerState_Release(sampler_linear_);
    if (sampler_nearest_)
        ID3D11SamplerState_Release(sampler_nearest_);

    // Rasterizer state
    if (raster_state_)
        ID3D11RasterizerState_Release(raster_state_);

    // Blend state
    if (blend_state_)
        ID3D11BlendState_Release(blend_state_);

    // Input layout
    if (input_layout_)
        ID3D11InputLayout_Release(input_layout_);

    // Shaders
    if (video_ps_)
        ID3D11PixelShader_Release(video_ps_);
    if (video_vs_)
        ID3D11VertexShader_Release(video_vs_);
    if (text_ps_)
        ID3D11PixelShader_Release(text_ps_);
    if (text_vs_)
        ID3D11VertexShader_Release(text_vs_);
    if (quad_ps_)
        ID3D11PixelShader_Release(quad_ps_);
    if (quad_vs_)
        ID3D11VertexShader_Release(quad_vs_);
    ...
    if (ctx_)
        ID3D11DeviceContext_Release(ctx_);
    if (swapchain_)
        IDXGISwapChain_Release(swapchain_);
    if (device_)
        ID3D11Device_Release(device_);
