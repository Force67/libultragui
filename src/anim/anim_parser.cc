...
static AnimProperty parse_property_name(const char* s) {
    ...
    if (std::strcmp(s, "w") == 0 || std::strcmp(s, "width") == 0) return AnimProperty::kW;
    if (std::strcmp(s, "h") == 0 || std::strcmp(s, "height") == 0) return AnimProperty::kH;
...
static AnimPropertyTrack parse_track(const char* prop_name, const JsonValue& arr) {
    ...
    for (auto& kf_val : arr.array_val) {
        ...
        if (auto* e = kf_val.get("ease")) kf.easing = parse_anim_easing(e->AsString());
...
static AnimLayer parse_layer(const JsonValue& obj) {
    ...
    if (auto* v = obj.get("shape")) layer.shape_type = parse_shape_type(v->AsString());
...
bool parse_anim_document(const JsonValue& root, AnimDocument& out) {
    ...
    if (auto* v = root.get("version")) out.version = static_cast<u32>(v->AsNumber(1));
