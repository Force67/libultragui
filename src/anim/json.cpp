...
const JsonValue* JsonValue::at(usize index) const {
    if (type != Array || index >= array_val.size()) return nullptr;
    return &array_val[index];
}

f64 JsonValue::as_number(f64 def) const {
    return type == Number ? number_val : def;
}

f32 JsonValue::as_float(f32 def) const {
    return type == Number ? static_cast<f32>(number_val) : def;
}

const char* JsonValue::as_string(const char* def) const {
    return type == String ? string_val.c_str() : def;
}
...
usize JsonValue::size() const {
    ...
    if (type == Object) return object_keys.size();
    return 0;
}
    ...
    bool parse_string(JsonValue& out) {
        ...
        while (p < end && *p != '"') {
            if (*p == '\\') {
                ...
                switch (*p) {
                case '"': out.string_val += '"'; break;
                case '\\': out.string_val += '\\'; break;
                case '/': out.string_val += '/'; break;
                case 'n': out.string_val += '\n'; break;
                case 't': out.string_val += '\t'; break;
                case 'r': out.string_val += '\r'; break;
                default: out.string_val += *p; break;
                }
            } else {
                out.string_val += *p;
            }
