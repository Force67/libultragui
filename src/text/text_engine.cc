...
struct GlyphKey {
  ...
  bool operator==(const GlyphKey& o) const {
    return font == o.font && glyph_id == o.glyph_id &&
...
void TextEngine::Shutdown() {
  ...
  for (u32 i = 0; i < MAX_FONTS; ++i) {
    if (impl_->fonts[i].in_use) {
      if (impl_->fonts[i].hb_font) hb_font_destroy(impl_->fonts[i].hb_font);
      if (impl_->fonts[i].ft_face) FT_Done_Face(impl_->fonts[i].ft_face);
    }
  }

  FT_Done_FreeType(impl_->ft_library);

  if (atlas_texture_ != kInvalidTexture) rhi_->DestroyTexture(atlas_texture_);
...
FontHandle TextEngine::ResolveFont(FontHandle base_font, FontWeight weight,
  ...
  if (family.empty()) return base_font;
  ...
  for (u32 i = 0; i < MAX_FONTS; ++i) {
    if (!impl_->fonts[i].in_use) continue;
    const auto& info = impl_->font_info_[i];
    if (info.family_name != family) continue;
    ...
    } else if (style_match == best_style_match &&
               weight_dist < best_weight_dist) {
      best = i;
      best_weight_dist = weight_dist;
    }
  }
...
TextRun TextEngine::Shape(FontHandle font, const char* text, u32 text_len,
  ...
  for (u32 i = 0; i < glyph_count; ++i) {
    ...
    // HarfBuzz positions are in FreeType 26.6 fixed-point at the DPI-scaled
    // size; scale back to window coords.
    ...
    g.x_advance = static_cast<f32>(glyph_pos[i].x_advance) / 64.0f * inv_dpi +
                  letter_spacing;
  ...
  // Font metrics from FreeType are at the DPI-scaled size; convert to window
  // coords.
  f32 ascent =
      static_cast<f32>(slot.ft_face->size->metrics.ascender) / 64.0f * inv_dpi;
  f32 descent =
      static_cast<f32>(slot.ft_face->size->metrics.descender) / 64.0f * inv_dpi;
...
TextLayout TextEngine::Layout(const TextRun& run,
  ...
  for (u32 i = 0; i < run.glyph_count; ++i) {
    ...
    if (line_x + advance > config.max_width && i > line_start) {
      ...
      impl_->scratch_lines.push_back(
          {line_start, break_at - line_start, break_width, 0});

      line_start = break_at;
      line_x = (break_at == last_break) ? (line_x - last_break_x + advance)
                                        : advance;
  ...
  if (line_start < run.glyph_count) {
    impl_->scratch_lines.push_back(
        {line_start, run.glyph_count - line_start, line_x, 0});
  }
...
void TextEngine::FlushAtlas() {
  if (!impl_->atlas_dirty) return;

  if (atlas_texture_ == kInvalidTexture) {
    // First time: create the texture (includes upload).
    // NEAREST filter prevents bilinear bleed between packed glyphs.
    atlas_texture_ =
        rhi_->CreateTexture(ATLAS_SIZE, ATLAS_SIZE, RHIFormat::kR8Unorm,
                            impl_->atlas_pixels, RHIFilter::kNearest);
  } else {
    // Subsequent: update in place (handle stays stable).
    rhi_->UpdateTexture(atlas_texture_, impl_->atlas_pixels);
  }
